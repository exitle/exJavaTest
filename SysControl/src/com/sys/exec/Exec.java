...
public class Exec {
	 ...
	 * 
	 * @param args
	 *            vol - SoundControl ( +x - increase by x -x -
	 *            decrease by x x - set to x )
	 * 
	 *            shut - Shutown (x - shutdown in x minutes -c - cancel
	 *            shutdown)
	 * 
	 *            ping - ping selected range
	 *            	(* | x-y | int | default: 192.168.0-1.*)
	 */
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			...
			case "shut":
				if (args.length > 1) {
					...
					System.out.printf("%s%n%s%n"
							, "Missing parameter."
							, "i.e. shut 60 - for shutdown in 60 minutes"
							);
				}
				break;
			case "ping":
			...
			default:
				System.out.printf("%s%n%s%n"
						, "Invalid argument. "
						, "Run with 'help' argument to list available options."
						);
				break;
			}
